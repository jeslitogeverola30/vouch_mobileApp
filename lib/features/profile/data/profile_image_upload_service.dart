...
class ProfileImageUploadService {
  ...
  static Future<String> uploadProfileImage({required File imageFile}) async {
    final uploadPreset = CloudinaryClient.profileUploadPreset;
    if (uploadPreset == null || uploadPreset.trim().isEmpty) {
      throw StateError(
        'Missing CLOUDINARY_PROFILE_UPLOAD_PRESET in .env file.',
      );
    }
    ...
    if (streamedResponse.statusCode < 200 ||
      ...
      throw Exception(
        errorMessage ?? 'Failed to upload profile image to Cloudinary.',
      );
    }
  ...
  static String _friendlyCloudinaryError(String message) {
    ...
    if (normalized.contains('whitelisted for unsigned uploads') ||
      ...
      return 'Cloudinary profile preset is not configured for unsigned uploads. Open Cloudinary Dashboard → Settings → Upload → Upload presets, edit your profile preset, set Signing mode to Unsigned, and save.';
    }

    if (normalized.contains('upload preset must be specified') ||
        normalized.contains('upload_preset')) {
      return 'Profile upload preset is invalid or missing. Check CLOUDINARY_PROFILE_UPLOAD_PRESET in .env.';
    }

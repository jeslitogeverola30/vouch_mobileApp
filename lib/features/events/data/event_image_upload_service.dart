...
class EventImageUploadService {
  ...
  static Future<String> uploadEventImage({required File imageFile}) async {
    final uploadPreset = CloudinaryClient.uploadPreset;
    if (uploadPreset == null || uploadPreset.trim().isEmpty) {
      throw StateError('Missing CLOUDINARY_UPLOAD_PRESET in .env file.');
    }
    ...
    if (streamedResponse.statusCode < 200 ||
      ...
      throw Exception(
        errorMessage ?? 'Failed to upload event image to Cloudinary.',
      );
    }
  ...
  static String _friendlyCloudinaryError(String message) {
    ...
    if (normalized.contains('whitelisted for unsigned uploads') ||
      ...
      return 'Cloudinary preset is not configured for unsigned uploads. Open Cloudinary Dashboard → Settings → Upload → Upload presets, edit your preset, set Signing mode to Unsigned, and save.';
    }

    if (normalized.contains('upload preset must be specified') ||
        normalized.contains('upload_preset')) {
      return 'Upload preset is invalid or missing. Check CLOUDINARY_UPLOAD_PRESET in .env.';
    }

...
CREATE OR REPLACE FUNCTION single_active_term() RETURNS TRIGGER AS $$ BEGIN IF NEW.is_active = TRUE THEN
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER ensure_single_active_term BEFORE
INSERT
    OR
UPDATE ON Academic_Terms FOR EACH ROW EXECUTE FUNCTION single_active_term();
-- ==============================================================================
...
SELECT 'Event' AS requirement_type,
    e.id AS source_id,
    e.name AS title,
...
UNION ALL
SELECT 'Payment' AS requirement_type,
    p.id AS source_id,
    p.title AS title,
...
VALUES ('2025-2026', '1st Semester', TRUE) ON CONFLICT (academic_year, semester) DO NOTHING;
